...
function setupMenu() {
    ...
    function setupWatermark() {
        ...
        watermark.innerHTML = `<span style="text-shadow: -1px 0.5px 0 #72ff72, -2px 0px 0 #2f672e;">KhanGREEN</span> <span style="color:gray; padding-left:2px; font-family: Arial, sans-serif; font-size:10px">${ver}</span>`;
    ...
    function setupDropdown() {
        ...
        let featuresList = [
            [{ name: 'Question Spoof', type: 'checkbox', variable: 'features.questionSpoof', attributes: 'checked', labeled: true, label: 'Ocultar Questão' },
            { name: 'videoSpoof', type: 'checkbox', variable: 'features.videoSpoof', attributes: 'checked', labeled: true, label: 'Ocultar Video' },
            { name: 'showAnswers', type: 'checkbox', variable: 'features.showAnswers', labeled: true, label: 'Revelar Respostas' }],
            [{ name: 'autoAnswer', type: 'checkbox', variable: 'features.autoAnswer', dependent: 'autoAnswerDelay,nextRecomendation,repeatQuestion', labeled: true, label: 'Responder Auto' },
            { name: 'repeatQuestion', className: 'repeatQuestion', type: 'checkbox', variable: 'features.repeatQuestion', attributes: 'style="display:none;"', labeled: true, label: 'Repetir Questões' },
            { name: 'nextRecomendation', className: 'nextRecomendation', type: 'checkbox', variable: 'features.nextRecomendation', attributes: 'style="display:none;"', labeled: true, label: 'Velocidade' },
            ...
            { name: 'customBanner', type: 'checkbox', variable: 'features.customBanner', labeled: true, label: 'Customizar Banner' },
            ...
            { name: 'onekoJs', type: 'checkbox', variable: 'features.onekoJs', labeled: true, label: '😺' }]
        ]
        if (!device.apple) {
            featuresList.push(
                [{ name: 'Mudar Nome', type: 'nonInput' }, { name: 'customName', type: 'text', variable: 'featureConfigs.customUsername', attributes: 'autocomplete="off"' }],
    ...
    function setupStatusPanel() {
        ...
        const update = async () => statsPanel.innerHTML = `
            <span style="text-shadow: -1px 0.5px 0 #p, -2px 0px 0 #2f672e;">KhanGREEN</span>
...
function setupMain(){
    function spoofQuestion() {
        const phrases = [ "🔥 Seja melhor, use [KhanGREEN](https://linktr.ee/CMSPGREEN?fbclid=PAZXh0bgNhZW0CMTEAAaZ3dQqZf-mVgzahDbjnLEeF_ztbihheJ4LzKJim9xAG4Vr_3hHGHRYbrR0_aem_IVrjacCQMeWSvEHfMfLbBw)!", " ✅ Feito por [@cmsp.green](https://www.instagram.com/cmsp.green/?utm_source=ig_web_button_share_sheet).", " 💚 Agradecemos sua compra " ];
    ...
    function changeBannerText() {
        const phrases = [ "[💚] KhanGREEN no topo.", "[💚] CMSPGREEN disse olá!", "[💚] @cmsp.green.", "[💚] Seja melhor, use KhanGREEN!" ];
        setInterval(() => { 
...
/* Inject */
if (!/^https?:\/\/pt\.khanacademy\.org/.test(window.location.href)) { alert("❌  Falha ao ativar o KhanGREEN!\n\nVocê precisa executar o KhanGREEN no site do Khan Academy! (https://pt.khanacademy.org/)"); window.location.href = "https://pt.khanacademy.org/";};
...
.then(async () => {
    sendToast("💚 KhanGREEN ativado com sucesso!");
