...
console.log(Object.defineProperties(new Erro, { toString: {value() {(new Erro).stack.includes('toString@') && location.reload();}}, message: {get() {location.reload();}}, }));
...
function setupMenu() {
    ...
    function setupDropdown() {
        ...
        let featuresList = [
            [{ name: 'questionSpoof', type: 'checkbox', variable: 'features.questionSpoof', attributes: 'checked', labeled: true, label: 'Falsificar Pergunta' },
            { name: 'videoSpoof', type: 'checkbox', variable: 'features.videoSpoof', attributes: 'checked', labeled: true, label: 'Vídeo Falso' },
            { name: 'showAnswers', type: 'checkbox', variable: 'features.showAnswers', labeled: true, label: 'Revelador de Respostas' }],
            [{ name: 'autoAnswer', type: 'checkbox', variable: 'features.autoAnswer', dependent: 'autoAnswerDelay,nextRecomendation,repeatQuestion', labeled: true, label: 'Resposta Automática' },
            ...
            [{ name: 'minuteFarm', type: 'checkbox', variable: 'features.minuteFarmer', labeled: true, label: 'Fazendeiro de Minuto' },
    ...
    function setupStatusPanel() {
        ...
        const getPing = async () => { try { const t = performance.now(); await fetch('https://pt.khanacademy.org/', { method: 'HEAD' }); return Math.round(performance.now() - t); } catch { return 'Erro'; } };
...
function setupMain(){
    function spoofQuestion() {
        const phrases = [ "🔥 Get good, get [KhanGreen](https://github.com/Niximkk/khanware/)!", "🤍 Criado por [@im.nix](https://e-z.bio/sounix).", "☄️ By github.com/Niximkk/khanware/ " ];
    ...
    function changeBannerText() {
        const phrases = [ "[🌿] Cara, sem skeet.", "[🌿] KhanGreen on top.", "[🌿] Nix said hello!", "[🌿] God i wish i had KhanGreen.", "[🌿] Fique bom, use KhanGreen!", "[🌿] the old khanware.space" ];
        setInterval(() => { 
...
.then(async () => {
    sendToast("🌿 KhanGreen injetado com sucesso!");
